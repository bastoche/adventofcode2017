...
def process_line(line, registers):
    ...
    if condition.is_true(registers):
        operation.perform(registers)


CONDITION_PATTERN = re.compile("if (\w+) ([<>=!]=?) (-?\d+)")
OPERATION_PATTERN = re.compile("(\w+) (inc|dec) (-?\d+)")


def parse_line(line):
    return parse_condition(line), parse_operation(line)


def parse_condition(line):
    result = CONDITION_PATTERN.search(line)
    register_name = result.group(1)
    comparison_operator = result.group(2)
    comparison_value = int(result.group(3))
    if comparison_operator == '>':
        return StrictlySuperiorCondition(register_name, comparison_value)
    elif comparison_operator == '>=':
        return SuperiorCondition(register_name, comparison_value)
    elif comparison_operator == '<':
        return StrictlyInferiorCondition(register_name, comparison_value)
    elif comparison_operator == '<=':
        return InferiorCondition(register_name, comparison_value)
    elif comparison_operator == '==':
        return EqualCondition(register_name, comparison_value)
    elif comparison_operator == '!=':
        return UnequalCondition(register_name, comparison_value)
...
def parse_operation(line):
    ...
    operation_value = int(result.group(3))
    if operator == 'inc':
        return IncrementOperation(register_name, operation_value)
    elif operator == 'dec':
        return DecrementOperation(register_name, operation_value)


class AbstractCondition(object):
    def __init__(self, register_name, comparison_value):
        self.register_name = register_name
        self.comparison_value = comparison_value

    def __eq__(self, other):
        if isinstance(self, other.__class__):
            return self.register_name == other.register_name and self.comparison_value == other.comparison_value
        return False


class StrictlySuperiorCondition(AbstractCondition):
    def is_true(self, registers):
        register_value = registers[self.register_name]
        return register_value > self.comparison_value


class SuperiorCondition(AbstractCondition):
    def is_true(self, registers):
        register_value = registers[self.register_name]
        return register_value >= self.comparison_value


class StrictlyInferiorCondition(AbstractCondition):
    def is_true(self, registers):
        register_value = registers[self.register_name]
        return register_value < self.comparison_value


class InferiorCondition(AbstractCondition):
    def is_true(self, registers):
        register_value = registers[self.register_name]
        return register_value <= self.comparison_value
...
class UnequalCondition(AbstractCondition):
    def is_true(self, registers):
        register_value = registers[self.register_name]
        return register_value != self.comparison_value
...
class AbstractOperation(object):
    def __init__(self, register_name, operation_value):
        self.register_name = register_name
        self.operation_value = operation_value

    def __eq__(self, other):
        if isinstance(self, other.__class__):
            return self.register_name == other.register_name and self.operation_value == other.operation_value
        return False


class IncrementOperation(AbstractOperation):
    def perform(self, registers):
        registers[self.register_name] += self.operation_value


class DecrementOperation(AbstractOperation):
    def perform(self, registers):
        registers[self.register_name] -= self.operation_value
